...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("cashflow:data");
        if (r?.value) {
          ...
          if (Array.isArray(d.avulsos)) setAvulsos(d.avulsos);
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await window.storage.set("cashflow:data",
          JSON.stringify({ receitas, despesas, saldoAnterior, filaOrder, pagos, ajustes, avulsos }));
      } catch (e) { /* silencioso */ }
    })();
  }, [receitas, despesas, saldoAnterior, filaOrder, pagos, ajustes, avulsos, loaded]);
  ...
  const resetSeed = () => {
    setReceitas(SEED_RECEITAS); setDespesas(SEED_DESPESAS); setSaldoAnterior(0); setPagos({}); setAjustes({}); setAvulsos([]);
  };
