...
const PAINEL_CARDS = [
  ...
  { key: "tempoMedio", label: "Tempo médio do cliente (meses)", icon: "⏱️", tipo: "manual", formato: "numero" },
];
...
function AppInner({ onLogout }) {
  ...
  }), [clientesAtivos, inadimplentes, totEntradasMes, totSaidasMes, receitaRecorrente, receitaMediaCliente, totalPipeline]);
                ...
                return (
                  <div key={c.key} style={S.painelCard}>
                    ...
                    <div style={S.painelCardVar}>
                      {auto ? (
                        <span style={{ color: "#4ade80" }}>{c.key === "entradas" ? "Mês atual" : c.key === "clientesAtivos" ? "recorrentes únicos" : c.key === "receitaRecorrente" ? "MRR (mensal)" : c.key === "receitaMedia" ? "por cliente ativo" : c.key === "lucro" ? "entradas − saídas" : c.key === "inadimplentes" ? "vencidos não pagos" : "automático"}</span>
                      ) : (
                        <EditCell
                          value={variacao}
                          muted
...
function EditCell({ value, onSave, type = "text", options, render, display, muted, align = "left", min, max, step, placeholder }) {
