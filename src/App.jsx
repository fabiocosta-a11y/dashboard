...
const SEED_RECEITAS = [
  ["Fresca Aromas","Pessoal","E-commerce","Pontual",1,375.01],
...
const SEED_DESPESAS = [
  ["Quebra-dedo 1 de 4","Pessoal","Extorsão","Semanal",5,1875,"Inegociável"],
  ["Quebra-dedo 2 de 4","Pessoal","Extorsão","Semanal",12,1875,"Inegociável"],
  ["Quebra-dedo 3 de 4","Pessoal","Extorsão","Semanal",19,1875,"Inegociável"],
  ["Quebra-dedo 4 de 4","Pessoal","Extorsão","Semanal",26,1875,"Inegociável"],
  ["Condomínio (2 de 2)","Pessoal","Residência","Mensal",10,1500,"Nice to have"],
  ["Condomínio (1 de 2)","Pessoal","Residência","Mensal",5,1500,"Nice to have"],
  ["Aluguel 4 de 4","Pessoal","Aluguel","Mensal",25,1500,"Nice to have"],
  ["Aluguel 3 de 4","Pessoal","Aluguel","Mensal",20,1500,"Nice to have"],
  ["Aluguel 2 de 4","Pessoal","Aluguel","Mensal",10,1500,"Nice to have"],
  ["Aluguel 1 de 4","Pessoal","Aluguel","Mensal",5,1500,"Nice to have"],
  ["Carro C3 - 3 de 3","Pessoal","Carros","Mensal",12,1000,"Inegociável"],
  ["Carro C3 - 2 de 3","Pessoal","Carros","Mensal",12,1000,"Inegociável"],
  ["Gás","Pessoal","Residência","Mensal",26,800,"Nice to have"],
  ...
  ["IPTU","Pessoal","Residência","Mensal",15,700,"Nice to have"],
  ["Convênio","Pessoal","Plano de Saúde","Mensal",5,700,"Nice to have"],
  ["Vivo - Celulares","Pessoal","Telefonia","Mensal",21,650,"Inegociável"],
  ["Microsoft Office (Agosto)","Pessoal","Ferramenta","Anual",26,599,"Nice to have"],
  ["Hostgator 1 (Agosto)","Costa Corp","Ferramenta","Anual",26,543.09,"Inegociável"],
  ["Claro - Internet de casa + Netflix","Pessoal","Internet","Mensal",20,520,"Inegociável"],
  ["Luz","Pessoal","Residência","Mensal",21,450,"Nice to have"],
  ["Mercado 3 de 4","Pessoal","Alimentação","Semanal",17,380,"Nice to have"],
  ["Mercado 2 de 4","Pessoal","Alimentação","Semanal",10,380,"Nice to have"],
  ["Mercado 1 de 4","Pessoal","Alimentação","Semanal",3,380,"Nice to have"],
  ["Contador (2 de 2)","Costa Corp","Contábil","Mensal",25,380,"Prioridade"],
  ["Contador (1 de 2)","Costa Corp","Contábil","Mensal",5,380,"Prioridade"],
  ["Mercado 4 de 4","Pessoal","Alimentação","Semanal",24,380,"Nice to have"],
  ["Carro C3 - 1 de 3","Pessoal","Carros","Mensal",12,366,"Inegociável"],
  ["Sem Parar","Pessoal","Carros","Mensal",29,300,"Prioridade"],
  ...
  ["Remédios","Pessoal","Remédio","Mensal",1,260,"Prioridade"],
  ["Feira 4 de 4","Pessoal","Alimentação","Semanal",27,200,"Prioridade"],
  ["Feira 3 de 4","Pessoal","Alimentação","Semanal",20,200,"Prioridade"],
  ["Feira 2 de 4","Pessoal","Alimentação","Semanal",13,200,"Prioridade"],
  ["Feira 1 de 4","Pessoal","Alimentação","Semanal",6,200,"Prioridade"],
  ...
  ["Vimeo","Pessoal","Ferramenta","Mensal",13,85,"Prioridade"],
  ["Domínios","Costa Corp","Ferramenta","Mensal",20,80,"Prioridade"],
  ["Apple iCloud","Pessoal","Ferramenta","Mensal",27,66.9,"Prioridade"],
  ["Make","Costa Corp","Ferramenta","Mensal",10,60,"Prioridade"],
  ["Vivo Easy","Costa Corp","Ferramenta","Mensal",15,55,"Prioridade"],
  ["YouTube Premium","Pessoal","Streaming","Mensal",6,55,"Prioridade"],
  ["(Apple) Instagram verificado","Pessoal","Ferramenta","Mensal",27,53.9,"Prioridade"],
  ["Disney +","Pessoal","Streaming","Mensal",9,50,"Prioridade"],
  ["Google One 2 TB","Pessoal","Ferramenta","Mensal",23,49.99,"Prioridade"],
  ["Max - HBO","Pessoal","Streaming","Trimestral",2,44.9,"Prioridade"],
  ["Spotify","Pessoal","Streaming","Mensal",1,40.9,"Prioridade"],
  ["Canva","Costa Corp","Ferramenta","Mensal",14,35,"Prioridade"],
  ["Apple TV+","Pessoal","Streaming","Mensal",3,29.9,"Prioridade"],
  ["Hospedagem - Gator Protect","Costa Corp","Ferramenta","Mensal",5,19.99,"Prioridade"],
  ["Amazon Prime","Pessoal","Streaming","Mensal",1,19.9,"Prioridade"],
  ["Amazon Prime Ad Free","Pessoal","Streaming","Mensal",26,10,"Prioridade"],
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (d.receitas) setReceitas(normalizarInicio(d.receitas));
        if (d.despesas) setDespesas(normalizarInicio(migrarQuebraDedo(d.despesas)));
        if (typeof d.saldoAnterior === "number") setSaldoAnterior(d.saldoAnterior);
        if (Array.isArray(d.filaOrder)) setFilaOrder(d.filaOrder);
        if (d.pagos && typeof d.pagos === "object") setPagos(d.pagos);
        if (d.ajustes && typeof d.ajustes === "object") setAjustes(d.ajustes);
      }
    } catch (e) { /* primeira vez: usa seed */ }
    setLoaded(true);
  }, []);
  ...
  useEffect(() => {
    if (!loaded) return;
    try {
      localStorage.setItem("cashflow:data",
        JSON.stringify({ receitas, despesas, saldoAnterior, filaOrder, pagos, ajustes }));
    } catch (e) { /* silencioso */ }
  }, [receitas, despesas, saldoAnterior, filaOrder, pagos, ajustes, loaded]);
  ...
  const receitasMes = useMemo(
    () => receitas.filter((r) => ativoNoMes(r, mes, ano)).map(aplicaAjuste),
    [receitas, mes, ano, ajustes]
  );
  const despesasMes = useMemo(
    () => despesas.filter((d) => ativoNoMes(d, mes, ano)).map(aplicaAjuste),
    [despesas, mes, ano, ajustes]
  );
  ...
  // ── CRUD ──────────────────────────────────────────────────
  const emptyRow = { desc: "", base: "Costa Corp", categoria: "", periodo: "Mensal", venc: 1, valor: 0, status: "Prioridade", mesInicio: mes, anoInicio: ano };
  ...
  const resetSeed = () => {
    setReceitas(SEED_RECEITAS); setDespesas(SEED_DESPESAS); setSaldoAnterior(0); setPagos({}); setAjustes({});
  };
  ...
  return (
    <div style={S.page}>
      <div style={S.shell}>
        ...
        <div style={S.header}>
          <div>
            <div style={S.brand}>COSTA CORP</div>
            <div style={S.subbrand}>Cash Flow · Fluxo de Caixa</div>
          </div>
        ...
        {/* Tabs */}
        <div style={S.tabs}>
          {[["fluxo","Fluxo de Caixa"],["fila","Fila de Priorização"],["cadastro","Cadastro"]].map(([k, l]) => (
        ...
        {tab === "fluxo" && (
          <div style={S.card}>
            ...
            {expand.entradas && WEEKS.some((w) => byWeek[w.key].itensR.length) && (
              <ExpandBlock weeks={WEEKS} byWeek={byWeek} field="itensR" isPago={isPago} togglePago={togglePago} onZerar={zerarNoMes} onRestaurar={restaurarValorMes} />
            )}
            ...
            />
            {expand.saidas && (
              <ExpandBlock weeks={WEEKS} byWeek={byWeek} field="itensD" showStatus isPago={isPago} togglePago={togglePago} onZerar={zerarNoMes} onRestaurar={restaurarValorMes} />
            )}
          ...
          <div style={S.card}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={S.table}>
                <thead>
                  <tr>
                    {["","Ordem","Descrição","Base","Categoria","Valor","Acumulado","Situação"].map((h, i) => (
                    ...
                    return (
                      <React.Fragment key={f.id}>
                        <tr style={{ opacity: f.cabe ? 1 : 0.5 }}>
                          ...
                          <td style={{ ...S.td, fontWeight: 600 }}>{f.desc}</td>
                          <td style={{ ...S.td, fontSize: 12, color: "#94a3b8" }}>{f.base}</td>
                          <td style={{ ...S.td, fontSize: 12, color: "#94a3b8" }}>{f.categoria}</td>
                          <td style={{ ...S.td, textAlign: "right" }}>{brl(f.valor)}</td>
                        ...
                        {cutoff && (
                          <tr><td colSpan={8} style={S.cutoff}>
            ...
            <div style={S.form}>
              ...
              <F label="Base">
                <select style={S.input} value={form.base} onChange={(e) => setForm({ ...form, base: e.target.value })}>
                  <option>Costa Corp</option><option>Pessoal</option>
                </select>
              </F>
              <F label="Categoria">
              ...
              <F label="Venc. (dia)">
                <input type="number" min={1} max={31} style={S.input} value={form.venc} onChange={(e) => setForm({ ...form, venc: e.target.value })} />
              </F>
              <F label="Início (mês)">
        ...
        <div style={S.footer}>Dados salvos automaticamente neste app · COSTA CORP · {MONTHS[mes]}/{ano}</div>
      </div>
    </div>
  );
}
...
function ExpandBlock({ weeks, byWeek, field, showStatus, isPago, togglePago, onZerar, onRestaurar }) {
          ...
          {weeks.map((w) => {
            ...
            return (
              <div key={w.key} style={S.expandCell}>
                {it ? (
                  <div style={{ ...S.expItem, opacity: pago ? 0.6 : 1, border: pago ? "1px solid rgba(74,222,128,.4)" : ajustado ? "1px solid rgba(251,191,36,.4)" : "1px solid transparent" }}>
                    <span style={S.expDesc}>{it.desc}</span>
...
const S = {
  ...
  cellInput: { width: "100%", boxSizing: "border-box", background: "rgba(10,16,38,.95)", border: "1px solid rgba(129,140,248,.5)", color: "#e2e8f0", padding: "4px 6px", borderRadius: 6, fontSize: 13, outline: "none" },
