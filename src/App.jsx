...
const PAINEL_CARDS = [
  ...
  { key: "inadimplentes", label: "Clientes inadimplentes", icon: "⚠️", tipo: "manual", formato: "numero" },
...
function AppInner({ onLogout }) {
  ...
  }), [clientesAtivos, totEntradasMes, totSaidasMes, receitaRecorrente, receitaMediaCliente, totalPipeline]);
              ...
              {PAINEL_CARDS.map((c) => {
                ...
                return (
                  <div key={c.key} style={S.painelCard}>
                    ...
                    <div style={{ ...S.painelCardVal, color: (c.key === "lucro" && (valor ?? 0) < 0) ? "#fb7185" : undefined }}>
                      {auto ? (
                    ...
                    <div style={S.painelCardVar}>
                      {auto ? (
                        <span style={{ color: "#4ade80" }}>{c.key === "entradas" ? "Mês atual" : c.key === "clientesAtivos" ? "recorrentes únicos" : c.key === "receitaRecorrente" ? "MRR (mensal)" : c.key === "receitaMedia" ? "por cliente ativo" : c.key === "lucro" ? "entradas − saídas" : "automático"}</span>
                      ) : (
                        <EditCell
                          value={variacao}
                          muted
