          ...
          <div style={S.card}>
            ...
            {leadsGanhos.length > 0 && (
              <div style={{ marginTop: 26 }}>
                <div style={{ ...S.listTitle, color: "#4ade80" }}>✓ Vendidos ({leadsGanhos.length}) · {brl0(totalGanhos)}</div>
                <div style={{ overflowX: "auto", marginTop: 8 }}>
                  <table style={S.table}>
                    <tbody>
                      {leadsGanhos.map((l) => (
                        <tr key={l.id} style={{ background: "rgba(74,222,128,.05)" }}>
                          <td style={{ ...S.td, fontWeight: 600 }}>{l.empresa || "—"}</td>
            ...
            {leadsPerdidos.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <div style={{ ...S.listTitle, color: "#fb7185" }}>✗ Perdidos ({leadsPerdidos.length}) · {brl0(totalPerdidos)}</div>
                <div style={{ overflowX: "auto", marginTop: 8 }}>
                  <table style={S.table}>
                    <tbody>
                      {leadsPerdidos.map((l) => (
                        <tr key={l.id} style={{ background: "rgba(251,113,133,.05)", opacity: 0.85 }}>
                          <td style={{ ...S.td, fontWeight: 600 }}>{l.empresa || "—"}</td>
                          <td style={S.td}>{l.nome || "—"}</td>
                          <td style={{ ...S.td, fontSize: 12, color: "#94a3b8" }}>{l.categoria}</td>
...
const S = {
  ...
  pipeGanhoBox: { background: "linear-gradient(135deg, rgba(34,197,94,.9), rgba(22,163,74,.9))", borderRadius: 12, padding: "10px 20px", textAlign: "right", minWidth: 150 },
  pipePerdaBox: { background: "linear-gradient(135deg, rgba(244,63,94,.9), rgba(190,18,60,.9))", borderRadius: 12, padding: "10px 20px", textAlign: "right", minWidth: 150 },
