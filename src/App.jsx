...
const PAINEL_CARDS = [
  ...
  { key: "conversao", label: "Taxa de conversão", icon: "✅", tipo: "manual", formato: "percent" },
...
function AppInner({ onLogout }) {
  ...
  const painelAuto = useMemo(() => ({
    ...
    cac: cacCalc,
  }), [clientesAtivos, inadimplentes, totEntradasMes, totSaidasMes, receitaRecorrente, receitaMediaCliente, totalPipeline, tempoMedioCliente, cacCalc]);
                    ...
                    <div style={S.painelCardVar}>
                      {auto ? (
                        c.key === "cac" ? (
                          ...
                          <span style={{ color: "#4ade80" }}>{c.key === "entradas" ? "Mês atual" : c.key === "clientesAtivos" ? "recorrentes únicos" : c.key === "receitaRecorrente" ? "MRR (mensal)" : c.key === "receitaMedia" ? "por cliente ativo" : c.key === "lucro" ? "entradas − saídas" : c.key === "inadimplentes" ? "vencidos não pagos" : c.key === "tempoMedio" ? "desde o 1º contrato" : "automático"}</span>
                        )
                      ) : (
                        <EditCell
                          value={variacao}
