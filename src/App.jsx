...
function AppInner({ onLogout }) {
  ...
  const clientesAtivos = useMemo(() => {
    ...
    const set = new Set();
    receitas.forEach((r) => {
      if (r.periodo !== "Pontual" && (Number(r.valor) || 0) > 0) {
  ...
  const tempoMedioCliente = useMemo(() => {
    ...
    const ativas = receitas.filter((r) => r.periodo !== "Pontual" && (Number(r.valor) || 0) > 0);
  ...
  // taxa de conversão = ganhos ÷ (ganhos + perdidos), em %
  const taxaConversao = useMemo(() => {
    const decididos = leadsGanhos.length + leadsPerdidos.length;
    if (decididos <= 0) return 0;
    return (leadsGanhos.length / decididos) * 100;
  }, [leadsGanhos, leadsPerdidos]);
              ...
              {novoTipo === "receita" && (
                <F label="Cliente desde (ano)">
                  <select style={S.input} value={form.clienteAno ?? form.anoInicio} onChange={(e) => setForm({ ...form, clienteAno: +e.target.value })}>
                    {[22, 23, 24, 25, 26, 27, 28].map((a) => <option key={a} value={a}>{a}</option>)}
                  </select>
                </F>
              )}
              {novoTipo === "despesa" && (
                    ...
                    <div style={S.painelCardVar}>
                      {auto ? (
                        c.key === "cac" ? (
                          ...
                          <span style={{ color: "#4ade80" }}>{c.key === "entradas" ? "Mês atual" : c.key === "clientesAtivos" ? "recorrentes únicos" : c.key === "receitaRecorrente" ? "MRR (mensal)" : c.key === "receitaMedia" ? "por cliente ativo" : c.key === "lucro" ? "entradas − saídas" : c.key === "inadimplentes" ? "vencidos não pagos" : c.key === "tempoMedio" ? "desde o 1º contrato" : c.key === "conversao" ? "ganhos ÷ decididos" : "automático"}</span>
                        )
                      ) : (
                        <EditCell
                          value={variacao}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                    ...
                    <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
                      ...
                      <EditCell value={r.anoInicio ?? 26} type="select" numericOpt
                        options={[22,23,24,25,26,27,28]} optionLabel={(v) => "/" + v}
